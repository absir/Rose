...
/*
	// <app_android_prj>/kdesktop/src/main/java/com/leagor/studio/app.java
	if (app_in_prj != copier_.app) {
		std::vector<std::string> javas;
		javas.push_back("app.java");
		if (copier_.bootup) {
			javas.push_back("BootCompletedReceiver.java");
		}
		for (std::vector<std::string>::const_iterator it = javas.begin(); it != javas.end(); ++ it) {
			const std::string& java = *it;
			ss.str("");
			ss << "/src/main/java/" + default_bundle_id.node(0) + "/" + default_bundle_id.node(1) + "/" + default_bundle_id.node(2) + "/" + java;
			tfile file(app_android_prj_app_path + ss.str(), GENERIC_WRITE, OPEN_EXISTING);
			int fsize = file.read_2_data();
			if (!fsize) {
				return false;
			}
			...
			std::string app_java_dir = app_android_prj_app_path + "/src/main/java/" + copier_.bundle_id.node(0) + "/" + copier_.bundle_id.node(1) + "/" + copier_.bundle_id.node(2);
			SDL_MakeDirectory(app_java_dir.c_str());
			tfile file2(app_java_dir + "/" + java, GENERIC_WRITE, CREATE_ALWAYS);
			posix_fwrite(file2.fp, file.data, fsize);
		}
		ss.str("");
		if (default_bundle_id.node(0) != copier_.bundle_id.node(0)) {
			ss << "/src/main/java/" + default_bundle_id.node(0);
		} else if (default_bundle_id.node(1) != copier_.bundle_id.node(1)) {
			ss << "/src/main/java/" + default_bundle_id.node(0) + "/" + default_bundle_id.node(1);
		} else {
			ss << "/src/main/java/" + default_bundle_id.node(0) + "/" + default_bundle_id.node(1) + "/" + default_bundle_id.node(2);
		}
		SDL_DeleteFiles((app_android_prj_app_path + ss.str()).c_str());
	}

	// <app_android_prj>/kdesktop/src/main/java/org/libsdl/app/UpdateService.java
	if (app_in_prj != copier_.app) {
		std::vector<std::pair<std::string, std::string> > replaces;
		replaces.push_back(std::make_pair(default_bundle_id.id(), copier_.bundle_id.id()));
		src = app_android_prj_app_path + "/src/main/java/org/libsdl/app/UpdateService.java";
