...
extern AdbInterfaceObject* InstantiateWinUsbInterface(const wchar_t* interface_name);
...
ADBAPIHANDLE __cdecl AdbCreateInterface(GUID class_id,
  ...
  if (0xFF == interface_id) {
    ...
    _swprintf(match_name, L"\\\\?\\usb#vid_%04x&pid_%04x#",
             vendor_id, product_id);
  } else {
    // With interface id for the name.
    _swprintf(match_name, L"\\\\?\\usb#vid_%04x&pid_%04x&mi_%02x#",
