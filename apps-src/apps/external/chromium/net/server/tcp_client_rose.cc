...
TcpClientRose::TcpClientRose(base::Thread& thread)
	: thread_(thread)
	, weak_ptr_factory_(this)
...
{
	VALIDATE_CALLED_ON_THIS_THREAD(client_->tid_);
	tauto_destruct_executor destruct_executor(boost::bind(&TcpClientRose::did_slice_quited, this, timeout));
...
void ttcp_manager::did_set_event()
{
	e_.Set();
}

void ttcp_manager::start_internal(const std::string& host, uint16_t port, int timeout)
{	
	tauto_destruct_executor destruct_executor(boost::bind(&ttcp_manager::did_set_event, this));
	delegate_.reset(app_create_delegate(*thread_.get()));
...
void ttcp_manager::stop_internal()
{
	tauto_destruct_executor destruct_executor(boost::bind(&ttcp_manager::did_set_event, this));
...
void ttcp_manager::start(const std::string& host, uint16_t port, int timeout)
{
	CHECK(thread_.get() == nullptr);
	thread_.reset(new base::Thread("TcpThread"));
	...
	thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&ttcp_manager::start_internal, base::Unretained(this), host, port, timeout));
	e_.Wait(rtc::Event::kForever);
}

void ttcp_manager::stop()
{
	...
	thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&ttcp_manager::stop_internal, base::Unretained(this)));
	e_.Wait(rtc::Event::kForever);

	thread_.reset();
}

bool ttcp_manager::xmit_transaction_internal(gui2::tprogress_& progress, const std::string& host, uint16_t port, const std::string& req, int timeout)
{
	std::string err_msg;
	...
	if (rose.connected_) {
		...
		rose.client_->did_write_layer((const uint8_t*)req.c_str(), req.size());
...
bool ttcp_manager::xmit_transaction(const std::string& host, uint16_t port, const std::string& req, int timeout)
{
	gui2::tprogress_default_slot slot(boost::bind(&ttcp_manager::xmit_transaction_internal, this, _1, host, port, boost::ref(req), timeout), null_str);
