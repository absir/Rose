...
{
	friend class ttcp_manager;
public:
	TcpClientRose(base::Thread& thread);
	~TcpClientRose();
...
public:
	...
	bool xmit_transaction(const std::string& host, uint16_t port, const std::string& req, int timeout);
...
private:
	...
	bool xmit_transaction_internal(gui2::tprogress_& progress, const std::string& host, uint16_t port, const std::string& req, int timeout);
	// impletement example: 
	//    return new TcpClientApp(thread);
	// TcpClientApp is derived from TcpClientRose.
	virtual TcpClientRose* app_create_delegate(base::Thread& thread) = 0;
