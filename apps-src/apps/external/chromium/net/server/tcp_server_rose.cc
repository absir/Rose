...
TcpServerRose::TcpServerRose(base::Thread& thread)
	: thread_(thread)
...
void TcpServerRose::tcpd_slice(int timeout)
{
	...
	tauto_destruct_executor destruct_executor(boost::bind(&TcpServerRose::did_slice_quited, this, timeout));
...
{
	tauto_destruct_executor destruct_executor(boost::bind(&ttcpd_manager::did_set_event, this));
