...
void tstack::set_did_pip_top_grid_rect(const tgrid& grid, const boost::function<SDL_Rect (tstack&, tgrid&, int width, int height)>& did)
{
	VALIDATE(mode_ == pip && grid_.children_vsize() > 1, null_str);
	VALIDATE(&grid != layer(0), null_str);

	std::map<const tgrid*, boost::function<SDL_Rect (tstack&, tgrid&, int width, int height)> >::iterator it = did_pip_top_grid_rects_.find(&grid);
	if (did != NULL) {
