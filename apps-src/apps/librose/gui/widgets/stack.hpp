...
public:
	...
	// pip
	void set_did_pip_top_grid_rect(const tgrid& grid, const boost::function<SDL_Rect (tstack&, tgrid&, int width, int height)>& did);
	void pip_relayout();
...
private:
	...
	std::map<const tgrid*, boost::function<SDL_Rect (tstack&, tgrid&, int width, int height)> > did_pip_top_grid_rects_;
