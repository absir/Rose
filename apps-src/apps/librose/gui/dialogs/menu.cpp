...
void tmenu::pre_show()
{
	...
	if (parent_) {
		if (motion_result_popup_) {
			window_->set_leave_dismiss(true, boost::bind(&tmenu::did_leave_dismiss, this, _1, _2));
		}
		window_->set_click_dismiss_except(boost::bind(&tmenu::did_click_dismiss_except, this, _1, _2));
	}
	...
	if (can_find_ && (int)items_.size() >= show_find_min_items_) {
		...
		text_box->set_did_text_changed(boost::bind(&tmenu::did_key_changed, this, _1));
	...
	list.set_did_row_changed(boost::bind(&tmenu::did_item_changed, this, _1, _2));
	list.set_did_row_focus_changed(boost::bind(&tmenu::did_item_focus_changed, this, _1, _2, _3));
}
...
{
	const int theory_max_show_items = 100;
