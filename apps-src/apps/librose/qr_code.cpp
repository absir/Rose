...
cv::Mat QrParse::FindQrPoint(const cv::Mat& bgr)
{
	...
	if (ppt_) {
		cvtColor(bgr, tmp, CV_BGR2BGRA);
		ppt_lock.reset(new tppt_lock(*this, tmp));
	}

	uint32_t start_ticks = SDL_GetTicks();

	cvtColor(bgr, gray, CV_BGR2GRAY);		// Convert Image captured from Image Input to GrayScale	

	if (ppt_) {
		cvtColor(gray, tmp, CV_GRAY2BGRA);
	...
	if (ppt_) {
		cvtColor(edges, tmp, CV_GRAY2BGRA);
	...
	findContours(edges, contours, hierarchy, CV_RETR_TREE, cv::CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
	...
	if (ppt_) {
		...
		findContours(unbolden_edges, contours2, hierarchy2, CV_RETR_TREE, cv::CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
		cvtColor(bgr, tmp, CV_BGR2BGRA);
	...
	if (qrAngle > 0.5) {
		...
		if (ppt_) {
			cvtColor(drawingRotation, tmp, CV_GRAY2BGRA);
